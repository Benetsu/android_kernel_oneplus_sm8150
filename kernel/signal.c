...
int group_send_sig_info(int sig, struct siginfo *info, struct task_struct *p)
{
	...
	if (!ret && sig) {
		...
		if (!ret && capable(CAP_KILL) && sig == SIGKILL &&
		    !strcmp(current->comm, ULMK_MAGIC)) {
			add_to_oom_reaper(p);
...
/**
 * do_freezer_trap - handle the cgroup v2 freezer jobctl trap
 *
 * Must be called with current->sighand->siglock held.  The lock is
